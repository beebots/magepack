...
module.exports = async (bundlingConfigPath, localesGlobPattern) => {
    ...
    // sort files from largest to smallest for each bundle
    bundleConfigMetaData.forEach(function(bundle, index){
        ...
        // end sort files from largest to smallest for each bundle
        bundle.modules = sortedModules;
    });
    let metaDataDir = path.dirname(bundlingConfigRealPath);
    let metaDataPath = metaDataDir + '/bundleMetaData.js';
    let contents = 'let metadata = ' + stringify(bundleConfigMetaData) + ';';
